...
// ══════════════════════════════════════════════════════════════════════════════
//  DOMBAG — Usuários (Consulta)
//  Operações: listar, redefinir senha, ativar/desativar, excluir
//  Cadastro e edição: /usuarios/cadastro (cad_usuarios.php)
// ══════════════════════════════════════════════════════════════════════════════
...
// ── DDL: garante colunas extras existem ──────────────────────────────────────
try {
...
$ativos  = count(array_filter($usuarios, fn($u) => ($u['USU_ATIVO'] ?? 1) == 1));
...
.usu-avatar    { width:32px;height:32px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;flex-shrink:0; }
...
@media(max-width:480px){ .td-actions{ flex-wrap:wrap; } }
</style>
</head>
<body>
  ...
  <div class="main">
    <header class="topbar">
      ...
      <div class="topbar-actions">
        <a href="/usuarios/cadastro" class="btn-primary">
    ...
    <div class="content">
      ...
      <?php if ($msg): ?>
      <div class="alert <?= $msg_tipo === 'ok' ? 'alert-ok' : 'alert-err' ?>" id="alertMsg" style="margin-bottom:16px;">
      ...
      <div class="kpi-strip" style="margin-bottom:20px;">
        ...
        <div class="kpi-mini c-teal">
          ...
          <div>
            <div class="kpi-mini-val"><?= $ativos ?></div>
        ...
        <div class="kpi-mini c-red">
          ...
          <div>
            <div class="kpi-mini-val"><?= $inativos ?></div>
      ...
      <div class="panel-table">
        ...
        <div class="empty-state">
          ...
          <p>Nenhum usuário cadastrado.<br>
             <a href="/usuarios/cadastro" class="btn-primary" style="margin-top:12px;display:inline-flex;">Criar primeiro usuário</a>
          </p>
        </div>
        <?php else: ?>
        <div class="table-wrap">
          <table>
            ...
            foreach ($usuarios as $u):
                ...
                $ativo  = (int)($u['USU_ATIVO'] ?? 1);
            ?>
            <tr id="row-u-<?= $u['USU_CODIGO'] ?>">
              <td>
                <div style="display:flex;align-items:center;gap:10px;">
                  <div class="usu-avatar" style="background:<?= $cor ?>20;color:<?= $cor ?>;"><?= $ini ?></div>
                  <div>
              ...
              <td class="td-muted"><?= $u['GRU_NOME'] ? htmlspecialchars($u['GRU_NOME']) : '—' ?></td>
              <td style="text-align:center;">
                <div class="toggle-wrap" style="justify-content:center;">
                  <label class="toggle" title="<?= $ativo ? 'Desativar' : 'Ativar' ?>">
                    <input type="checkbox" <?= $ativo ? 'checked' : '' ?>
                           onchange="toggleAtivo(<?= $u['USU_CODIGO'] ?>, this.checked)">
              ...
              <td>
                <div class="td-actions">
                  <a href="/usuarios/cadastro?id=<?= $u['USU_CODIGO'] ?>" class="btn-sm btn-sm-edit">
...
async function toggleAtivo(cod, ativo) {
  ...
  try {
    ...
    if (!data.ok) console.warn('toggle falhou para cod='+cod);
