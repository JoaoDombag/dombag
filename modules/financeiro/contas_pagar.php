...
// ── Contas a Pagar ERP Yzidro (PostgreSQL) ────────────────────────────────────
...
$yzRows   = [];
$yzErp    = '';
$yzTotals = ['a_pagar' => 0.0, 'pago' => 0.0, 'vencido' => 0.0];
...
try {
    ...
    if ($pg) {
        ...
        if ($yzStatus !== '') {
            if ($yzStatus === 'Pago') {
                $extras[] = "COALESCE(CP.CP_SALDO_MP, 0) <= 0";
            } else {
                $extras[] = "COALESCE(CP.CP_SALDO_MP, 0) > 0";
            }
        }
        if ($yzQ !== '') {
            $params[] = '%' . $yzQ . '%';
            $idx       = count($params);
            $extras[]  = "(UPPER(FR.FOR_NOME) ILIKE \${$idx} OR UPPER(CP.CP_OPERACAO) ILIKE \${$idx} OR UPPER(CP.CP_NUM_DOCUMENTO) ILIKE \${$idx})";
        }
        ...
        $sql = "
            SELECT CP.CP_CODIGO,
                   ...
                   COALESCE(CP.CP_COMPLEMENTO,  '')                                    AS complemento,
                   TO_CHAR(CP.CP_DATA_EMISSAO, 'YYYY-MM-DD')                          AS emissao,
                   TO_CHAR(CP.CP_VENCIMENTO,   'YYYY-MM-DD')                          AS vencimento,
                   ...
                   CAST(COALESCE(CP.CP_VALOR_RECEBIDO_MP, 0) AS NUMERIC(18,2))        AS valor_baixa,
                   CAST(COALESCE(CP.CP_SALDO_MP,          0) AS NUMERIC(18,2))        AS saldo_parcela,
                   COALESCE(FR.FOR_NOME,     '')                                       AS nome_fornecedor,
                   COALESCE(FR.FOR_CNPJ,     '')                                       AS cnpj_fornecedor,
            ...
            LEFT JOIN FORNECEDOR   FR ON FR.FOR_CODIGO = CP.FOR_CODIGO
            LEFT JOIN PLANO_CONTAS PC ON PC.CO_CODIGO  = CP.CO_CODIGO
            LEFT JOIN CENTROCUSTO  CC ON CC.CC_CODIGO  = CP.CC_CODIGO
            WHERE CP.CP_TIPO      = 'D'
              AND CP.CP_VINCULO   IS NULL
              AND CP.EMP_CODIGO   = 1
              AND CP.CP_VENCIMENTO >= \$1
              AND CP.CP_VENCIMENTO <= \$2
              {$extraWhere}
            ORDER BY CP.CP_VENCIMENTO ASC, CP.CP_CODIGO ASC
        ";
        ...
        if ($res) {
            while ($row = pg_fetch_assoc($res)) {
                ...
                $venc  = $row['vencimento'];
                $st    = $row['status_desc'];
                if ($st === 'Pago')     $yzTotals['pago']    += $val;
                elseif ($venc < $hoje)  $yzTotals['vencido'] += $saldo;
                else                    $yzTotals['a_pagar'] += $saldo;
            }
            pg_free_result($res);
        } else {
...
.yz-st-apagar  { background:rgba(45,106,255,.1);  color:#7db3ff;    padding:3px 9px; border-radius:999px; font-size:11px; font-weight:700; white-space:nowrap; }
.yz-st-pago    { background:rgba(16,185,129,.12); color:#10b981;   padding:3px 9px; border-radius:999px; font-size:11px; font-weight:700; white-space:nowrap; }
...
<header class="topbar">
  <div class="topbar-left">
    <div class="page-title">
      ...
      <p>Relatório de lançamentos a pagar — ERP Yzidro.</p>
    </div>
  </div>
</header>
...
<div class="content">
  ...
  <div class="sum-bar">
    <div class="sum-chip">
      <div class="sum-chip-val" style="color:#7db3ff;"><?= finMoney($yzTotals['a_pagar']) ?></div>
      <div class="sum-chip-lbl">A Pagar</div>
    </div>
    <div class="sum-chip">
    ...
    <div class="sum-chip">
      <div class="sum-chip-val" style="color:#10b981;"><?= finMoney($yzTotals['pago']) ?></div>
      ...
      <div class="filter-bar">
        ...
        <div class="field">
          ...
          <select name="yz_status">
            ...
            <?php foreach (['A Pagar','Pago'] as $st): ?>
        ...
        <div style="display:flex;gap:8px;align-items:flex-end;">
          ...
          <a href="/financeiro/pagar" class="btn-secondary" style="height:34px;display:inline-flex;align-items:center;">Limpar</a>
        </div>
      </div>
    </form>
  </div>
      ...
      <table class="fin-tbl">
        <thead>
          <tr>
            ...
            <th style="text-align:right">Baixa</th>
        ...
        ?>
          <tr class="<?= $atrasado ? 'vencido-row' : '' ?>">
            ...
            <td style="font-size:12px;">
              <div><?= finH($r['nome_fornecedor'] ?: '—') ?></div>
            ...
            <td style="font-size:11.5px;"><?= finH($r['centro_custo'] ?: '—') ?></td>
