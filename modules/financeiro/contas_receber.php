...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'salvar') {
            ...
            $cliente = trim((string) ($_POST['cliente_nome'] ?? ''));
            $cat     = trim((string) ($_POST['categoria'] ?? 'Venda'));
            ...
            if ($id > 0) {
                $pdo->prepare('UPDATE fin_contas_receber SET descricao=:d, cliente_nome=:c, categoria=:cat, valor=:v, data_emissao=:e, data_vencimento=:venc, observacoes=:obs WHERE id=:id')
                    ->execute([':d'=>$desc,':c'=>$cliente,':cat'=>$cat,':v'=>$valor,':e'=>$emissao,':venc'=>$venc,':obs'=>$obs,':id'=>$id]);
            } else {
                $pdo->prepare('INSERT INTO fin_contas_receber (descricao,cliente_nome,categoria,valor,data_emissao,data_vencimento,observacoes) VALUES (:d,:c,:cat,:v,:e,:venc,:obs)')
                    ->execute([':d'=>$desc,':c'=>$cliente,':cat'=>$cat,':v'=>$valor,':e'=>$emissao,':venc'=>$venc,':obs'=>$obs]);
            }
            ...
            $row = $pdo->prepare('SELECT valor FROM fin_contas_receber WHERE id=?');
            $row->execute([$id]);
            $r = $row->fetch();
            if (!$r) { echo json_encode(['ok'=>false,'msg'=>'Registro não encontrado.']); exit; }

            $status = finResolveStatus((float)$r['valor'], $valorPago, $dataPag);
            $pdo->prepare('UPDATE fin_contas_receber SET valor_pago=:vp, data_pagamento=:dp, status=:s WHERE id=:id')
                ->execute([':vp'=>$valorPago,':dp'=>$dataPag ?: null,':s'=>$status,':id'=>$id]);
            echo json_encode(['ok' => true]);

        } elseif ($action === 'cancelar') {
            $id = (int) ($_POST['id'] ?? 0);
            $pdo->prepare('UPDATE fin_contas_receber SET status=:s WHERE id=:id')
                ->execute([':s'=>'CANCELADO',':id'=>$id]);
            echo json_encode(['ok' => true]);

        } elseif ($action === 'excluir') {
            $id = (int) ($_POST['id'] ?? 0);
            $pdo->prepare('DELETE FROM fin_contas_receber WHERE id=?')->execute([$id]);
            echo json_encode(['ok' => true]);
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    $rows = $pdo->query('SELECT descricao,cliente_nome,categoria,valor,valor_pago,data_emissao,data_vencimento,data_pagamento,status,observacoes,criado_em FROM fin_contas_receber ORDER BY data_vencimento DESC')->fetchAll();
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="contas_receber_' . date('Y-m-d') . '.csv"');
...
if ($fBusca  !== '') {
    $where[] = '(descricao LIKE ? OR cliente_nome LIKE ? OR categoria LIKE ?)';
    $like = '%'.$fBusca.'%';
    array_push($params, $like, $like, $like);
}
if ($fMes !== '') { $where[] = 'DATE_FORMAT(data_vencimento,"%Y-%m") = ?'; $params[] = $fMes; }

$sql = 'SELECT * FROM fin_contas_receber WHERE ' . implode(' AND ', $where) . ' ORDER BY data_vencimento ASC, id DESC';
...
<header class="topbar">
  <div class="topbar-left">
    <div class="page-title">
      ...
      <p>Gerencie seus recebimentos e controle vencimentos.</p>
    </div>
  </div>
...
<div class="content">
  ...
  <div class="sum-bar">
    <div class="sum-chip">
      <div class="sum-chip-val" style="color:#7db3ff;"><?= finMoney($totalAberto) ?></div>
      <div class="sum-chip-lbl">Em aberto</div>
    </div>
    <div class="sum-chip">
      <div class="sum-chip-val" style="color:var(--red);"><?= finMoney($totalVencido) ?></div>
      <div class="sum-chip-lbl">Vencido</div>
    </div>
    <div class="sum-chip">
      <div class="sum-chip-val" style="color:#10b981;"><?= finMoney($totalPago) ?></div>
  ...
  <div class="panel" style="margin-bottom:20px;">
    <form method="GET" action="">
      <div class="filter-bar">
        <div class="field">
          <label>Busca</label>
          <input type="text" name="q" value="<?= finH($fBusca) ?>" placeholder="Descrição, cliente, categoria…">
        </div>
        <div class="field">
          <label>Status</label>
          <select name="status">
        ...
        <div style="display:flex;gap:8px;align-items:flex-end;">
          ...
          <a href="/financeiro/receber" class="btn-secondary" style="height:36px;display:inline-flex;align-items:center;">Limpar</a>
        </div>
      </div>
    </form>
  </div>
    ...
    <div style="overflow-x:auto;">
      <table class="fin-tbl">
        ...
        <?php foreach ($rows as $r):
          ...
          $saldoR   = (float)$r['valor'] - (float)$r['valor_pago'];
        ?>
          <tr id="row-<?= $r['id'] ?>" class="<?= $atrasado ? 'vencido-row' : '' ?>">
            ...
            <td style="font-size:12px;"><?= finH($r['cliente_nome'] ?: '—') ?></td>
            ...
            <td>
              <div class="row-actions">
                ...
                <button class="btn-icon success" title="Registrar recebimento" onclick="abrirPagamento(<?= $r['id'] ?>, <?= $r['valor'] ?>, <?= $r['valor_pago'] ?>)">
...
<div class="modal-backdrop hidden" id="modalBackdrop" onclick="fecharModal(event)">
  <div class="modal" onclick="event.stopPropagation()">
    <div class="modal-title" id="modalTitulo">Nova Conta a Receber</div>
    <input type="hidden" id="mId" value="0">
    <div class="modal-grid">
      <div class="field span-2">
        <label>Descrição *</label>
        <input type="text" id="mDesc" placeholder="Ex.: Fatura #123 — Cliente ABC">
      </div>
      <div class="field">
        <label>Cliente</label>
      ...
      <div class="field">
        ...
        <select id="mCat">
          <?php foreach (FIN_CATEGORIAS_RECEBER as $c): ?>
...
<script>
const URL = window.location.pathname + window.location.search.replace(/[?&](export|action)=[^&]*/g,'');

function abrirModal(id=0) {
  document.getElementById('mId').value = id;
  document.getElementById('modalTitulo').textContent = id ? 'Editar Conta' : 'Nova Conta a Receber';
  if (!id) {
...
function editarLancamento(r) {
  ...
  document.getElementById('mCliente').value = r.cliente_nome || '';
  document.getElementById('mCat').value     = r.categoria || 'Venda';
...
async function salvarModal() {
  ...
  fd.append('cliente_nome',    document.getElementById('mCliente').value);
